...
export class HttpLoggerMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    this.logger.log(`请求参数: query=${JSON.stringify(query)}, params=${JSON.stringify(params)}, body=${JSON.stringify(body)}`);
    
    // 记录响应
    const originalSend = res.send;
    res.send = function(responseBody) {
      const responseTime = new Date().getTime() - (requestWithTime.startTime || 0);
      const logContext = `${method} ${originalUrl} ${res.statusCode} ${responseTime}ms`;
      
      try {
        if (res.statusCode >= 400) {
          this.logger.error(`错误响应: ${responseBody}`, logContext);
          this.logger.error(`请求详情: method=${method}, url=${originalUrl}, headers=${JSON.stringify(headers)}, body=${JSON.stringify(body)}`);
        } else {
