...
export class JwtStrategy extends PassportStrategy(Strategy) {
  constructor(
    ...
    if (!jwtSecret) {
      ...
      console.error('JWT_SECRET is not defined in environment variables. Please check your .env file.');
      throw new InternalServerErrorException('JWT secret is missing. Server configuration error.');
    }
    super({
  ...
  async validate(payload: { sub: number; openid: string }): Promise<Partial<User>> {
    // payload 结构: { sub: userId (number), openid: userOpenid (string), iat: ..., exp: ... }
    // console.log('[JwtStrategy] Validating payload:', payload);
    if (!payload || !payload.sub) {
        throw new UnauthorizedException('Invalid token payload: missing subject (user ID).')
    }
